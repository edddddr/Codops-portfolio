...
def reverseCompare(nums):
    
    rev = ''
    splited_num = list(str(nums))

    splited_num.reverse()

    for s in splited_num:
        rev += s
...
def checkMeera(meera_list):
    
    checker = [m * 2 for m in meera_list]

    for c in range(len(checker)):
        for m in range(len(meera_list)):
            if checker[c] == meera_list[m]:
...
def digital_clock(seconds: int) -> str:
    # Use 86400 seconds in a day to handle next-day rollover
